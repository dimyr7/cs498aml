...
def deepnn(x):
  ...
  with tf.name_scope("conv3"):
      W_cov3 = weight_variable([5,5,8,16])
  ...
  with tf.name_scope("fc1"):
    ...
    W_fc1 = weight_variable([16 * 16 * 16, 256])
    b_fc1 = bias_variable([256])

    h_pool2_flat = tf.reshape(h_conv3, [-1, 16 * 16 * 16])
    ...
    with tf.Session() as sess:
        ...
        for i in range(2000):
            ...
            if i % 100 == 0:
                ...
                summary, _ = sess.run([merged_summary, train_step], feed_dict=feed_dict(True))
