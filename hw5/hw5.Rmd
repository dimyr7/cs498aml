...
print(c("The minimum log lambda is", log(lat.l1$lambda.min)))
plot(lat.l1)
```
...
print(c("The minimum log lambda is", log(lat.l2$lambda.min)))
plot(lat.l2)
```
...
xyplot((lon.lm$residuals)^(1/lon.bc_lambda) ~ (lon.lm$fitted.values)^(1/lon.bc_lambda),
      xlab = "Fitted Values",
      ylab = "Residuals",
      main = "Residual Diagnostic Plot",
	  sub = toString(c("Power scale", lat.bc_lambda)),
...
```{r}
cancer.lm_cv_auc <- cv.glmnet(model.matrix(~., data=cancer[1:cancer.num_features]),
						as.matrix(cancer[cancer.num_features+1]),
						family="binomial",
						alpha=1,
						type.measure ="auc",
						nfolds=5)
cancer.lm_cv_deviance <- cv.glmnet(model.matrix(~., data=cancer[1:cancer.num_features]),
						as.matrix(cancer[cancer.num_features+1]),
						family="binomial",
						type.measure = "deviance",
						alpha=1)
plot(cancer.lm_cv_auc, sub="auc")
plot(cancer.lm_cv_deviance, sub="deviance")
```
